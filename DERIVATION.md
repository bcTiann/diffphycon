...
$$x_\tau = \alpha_\tau z + \beta_\tau \varepsilon, \qquad \alpha_\tau = \tau,\ \beta_\tau = 1 - \tau.$$
...
$$\nabla_{(u, w)} \log p(w \mid c) \;=\; \big[\, 0, \ \nabla_w \log p(w \mid c) \,\big].$$

**Step 4.** Apply Proposition 1 to the prior $p_\tau(w \mid c)$ (**using the same path** as the joint):
$$\nabla_w \log p_\tau(w \mid c) \;=\; \frac{1}{a_\tau} \big[\, u_\tau^{\text{prior}}(w \mid c) - b_\tau w \,\big].$$

Substituting into Step 2, the $a_\tau$ factors cancel exactly:

$$\boxed{\ \tilde u_\tau^{\text{rw}} \;=\; u_\tau^{\text{target}} \;+\; (\gamma - 1) \big[\, u_\tau^{\text{prior}} - b_\tau [0, w] \,\big] \ }$$
...
$$\boxed{\ \tilde u_\tau^{\text{rw}} \;=\; u_\tau^{\text{target}} \;+\; (\gamma - 1)\, \tilde\eta(\tau)\, \big[\, u_\tau^{\text{prior}} - b_\tau [0, w] \,\big] \ }$$
