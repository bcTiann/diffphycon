...
class EllipseBody extends Body {
  ...
  EllipseBody( float x, float y, float _h, float _a, Window window) {
    ...
    // xc.x = xc.x - dx; // shift center to one end of ellipse
    xc.x = xc.x - dx + 1.0; 
    end(); // finalize shape
...
//  simple elliptical body extension
//  ********************************/
// class RotatedEllipseBody extends Body {
//   float h, a, alpha; // height and aspect ratio, and rotation_angle of ellipse
//   int m = 40;

//   EllipseBody( float x, float y, float _h, float _a, float alpha, Window window) {
//     super(x, y, window);
//     h = _h; 
//     a = 1./_a;
//     float dx = 0.5*h*a, dy = 0.5*h;
